...
export default function AdminDashboard() {
  ...
  const importObjective13BisMutation = trpc.admin.importObjective13Bis.useMutation();
  ...
  const handleImportObjective13Bis = async () => {
    setFormError('');
    setSuccessMessage('');
    try {
      const result = await importObjective13BisMutation.mutateAsync({ fileName: 'of_cap_objetivo_1_3_bis.json' });
      setSuccessMessage(`Importação concluída: ${result.imported} questões em ${result.chapters.join(', ')}.`);
    } catch (error) {
      setFormError(error instanceof Error ? error.message : 'Não foi possível importar o objetivo 1.3 bis.');
    }
  };
      ...
      <main className="mx-auto max-w-6xl space-y-8 px-4 py-8">
        ...
        <Card>
          ...
          <CardContent className="flex flex-wrap items-center gap-3">
            <Button type="button" variant="outline" onClick={handleImportObjective13Bis} disabled={importObjective13BisMutation.isPending}>
              {importObjective13BisMutation.isPending ? 'Importando OF 1.3 bis…' : 'Importar OF 1.3 bis (400 questões)'}
            </Button>
