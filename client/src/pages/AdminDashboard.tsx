...
export default function AdminDashboard() {
  ...
  const importObjective13Mutation = trpc.admin.importObjective13.useMutation();
  ...
  const handleImportObjective13 = async () => {
    setFormError('');
    setSuccessMessage('');
    try {
      const result = await importObjective13Mutation.mutateAsync({ fileName: 'of_cap_objetivo_1_3.json' });
      setSuccessMessage(`Importação concluída: ${result.imported} questões em ${result.chapters.join(', ')}.`);
    } catch (error) {
      setFormError(error instanceof Error ? error.message : 'Não foi possível importar o objetivo 1.3.');
    }
  };
      ...
      <main className="mx-auto max-w-6xl space-y-8 px-4 py-8">
        ...
        <Card>
          ...
          <CardContent className="flex flex-wrap items-center gap-3">
            <Button type="button" variant="outline" onClick={handleImportObjective13} disabled={importObjective13Mutation.isPending}>
              {importObjective13Mutation.isPending ? 'Importando OF 1.3…' : 'Importar OF 1.3 (541 questões)'}
            </Button>
