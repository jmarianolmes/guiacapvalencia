        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleCreateUser} className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2"><Label htmlFor="new-name">{text.name}</Label><Input id="new-name" value={name} onChange={event => setName(event.target.value)} disabled={createUserMutation.isPending} /></div>
              <div className="space-y-2"><Label htmlFor="new-email">{text.email}</Label><Input id="new-email" type="email" value={email} onChange={event => setEmail(event.target.value)} disabled={createUserMutation.isPending} required /></div>
              <div className="space-y-2"><Label htmlFor="temporary-password">{text.temporaryPassword}</Label><div className="flex gap-2"><Input id="temporary-password" type={showTemporaryPassword ? 'text' : 'password'} minLength={8} value={temporaryPassword} onChange={event => setTemporaryPassword(event.target.value)} disabled={createUserMutation.isPending} required /><Button type="button" variant="outline" size="sm" onClick={() => setShowTemporaryPassword(value => !value)}>{showTemporaryPassword ? text.hidePassword : text.showPassword}</Button></div></div>
