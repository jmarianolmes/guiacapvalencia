            ...
            {(reviewReportsQuery.data || []).length === 0 ? <p className="text-sm text-slate-500">{text.reviewEmpty}</p> : (reviewReportsQuery.data || []).map(({ report, question }) => (
              <div key={report.id} className="rounded-lg border border-amber-200 bg-amber-50 p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2 rounded-md border border-blue-200 bg-blue-50 p-3">
                  ...
                  <Button size="sm" variant="outline" disabled={officialLookupMutation.isPending} onClick={() => handleCompareOfficial(question.id)}>{text.compareOfficial}</Button>
                  ...
                  {officialSuggestions[question.id]?.answer && <Button size="sm" onClick={() => resolveReviewMutation.mutate({ reportId: report.id, correctAnswer: officialSuggestions[question.id]!.answer! }, { onSuccess: (data) => { setOfficialSuggestions((current) => { const next = { ...current }; delete next[question.id]; return next; }); setSuccessMessage(`Correção aplicada. ${data.recalculatedResults} resultado(s) histórico(s) recalculado(s).`); reviewReportsQuery.refetch(); } })}>{text.acceptOfficial}</Button>}
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  <Button size="sm" disabled={!reviewAnswers[report.id] || resolveReviewMutation.isPending} onClick={() => resolveReviewMutation.mutate({ reportId: report.id, correctAnswer: reviewAnswers[report.id] }, { onSuccess: (data) => { setSuccessMessage(`Correção aplicada. ${data.recalculatedResults} resultado(s) histórico(s) recalculado(s).`); reviewReportsQuery.refetch(); } })}>{text.reviewResolve}</Button>
